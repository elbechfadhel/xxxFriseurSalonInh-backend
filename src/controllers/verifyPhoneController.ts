...
const M = {
    ...
    smsBody: (lang: Lang, code: string, minutes: number) => {
        if (lang === "en") {
            return `${code} is your XXXFriseursalon Inh. Najib Neffati verification code. Valid for ${minutes} minutes. Do not share it.`;
        }

        // 🇩🇪 Allemand
        if (USE_ASCII_SMS) {
            return `${code} ist Ihr XXXFriseursalon Inh. Najib Neffati Bestaetigungscode. Gueltig ${minutes} Minuten. Bitte nicht weitergeben.`;
        } else {
            return `${code} ist Ihr XXXFriseursalon Inh. Najib Neffati Bestätigungscode. Gültig ${minutes} Minuten. Bitte nicht weitergeben.`;
        }
    },
};
